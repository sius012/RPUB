...
function isMouseInsideBox(mouseX, mouseY, box) {
    return (
        mouseX >= box.boundingPosition.x &&
        mouseX <= box.boundingPosition.x + box.width &&
        mouseY >= box.boundingPosition.y &&
        mouseY <= box.boundingPosition.y + box.height
    );
}
...
class Timechart {
    ...
    init() {
        this.dataView = this.data.map(function (e) {
            let ds = new Dataset();
            ds.data = e;
            return ds;
        });
    ...
    renderDays() {
        ...
        //Render Data
        master.dataView.forEach(function (e, i) {
            let datae = e.data;
            let tanggalStart = new Date(datae[master.config.start_date]);
            let tanggalEnd = new Date(datae[master.config.end_date]);
            ...
            master.ctx.fillStyle = e.currentColor;
            master.ctx.beginPath();
            master.ctx.roundRect(
                offset + master.offsetX,
                60 * i + 100,
                len,
                40,
                [10]
            ...
            master.ctx.fillText(
                datae[master.config.title],
                offset + master.offsetX + 5,
                60 * i + 125
            );
        });
        master.lastPasses = passess;

        if (master.hoveredElement != null) {
            let offsetX = master.hoveredElement.boundingPosition.x;
            let offsetY = master.hoveredElement.boundingPosition.y;
            console.log(master.hoveredElement.data.nama);

            master.ctx.beginPath();
            master.ctx.fillStyle = "black";
            master.ctx.roundRect(
                master.hoveredElement.boundingPosition.x,
                master.hoveredElement.boundingPosition.y - 40,
                50,
                40,
                [10]
            );
            master.ctx.fill();
            Object.keys(master.hoveredElement.data).forEach((key, index) => {
                let data = master.hoveredElement.data[key];
                master.ctx.beginPath();
                master.ctx.fillStyle = "blue";
                master.ctx.font = "12pt  Arial";
                master.ctx.fillText("halo", offsetX, offsetY + 50 * index, 20);
            });
        }
    }
    ...
    globalEvent() {
        ...
        this.canvas.addEventListener("wheel", function (e) {
            if (e.ctrlKey) {
                e.preventDefault();
                master.currentWidthItem =
                    master.currentWidthItem + e.deltaY / 20;
                master.renderDays();
            }
        ...
        this.canvas.addEventListener("mousemove", function (event) {
            const mouseX =
                event.clientX - master.canvas.getBoundingClientRect().left;
            const mouseY =
                event.clientY - master.canvas.getBoundingClientRect().top;
            console.log(mouseX);
            master.dataView.forEach(function (e) {
                if (isMouseInsideBox(mouseX, mouseY, e)) {
                    // e.currentColor = "blue";
                    master.hoveredElement = e;
                    console.log(master.hoveredElement);
                } else {
                    e.currentColor = e.color;
                    // master.hoveredElement = null;
                }
            });
            master.renderDays();
        });
